...
The log subsystem was split (2026-06-06): core keeps a small emit surface — `thx::logMessage(level, msg)` + `thx::assertThat` + a `LogSink` C function-pointer slot (`thx::setSink`, stderr fallback), all flat in the `thx::` namespace — in [include/thx/log.h](include/thx/log.h), the minimum core needs for its own diagnostics (`thx::rtti::SourceLocation` lives in [include/thx/rtti/source_location.h](include/thx/rtti/source_location.h)). The emit function is `logMessage`, not `log`, so the `thx::log` name stays free for the subsystem namespace. The service-based logging moved out to [plugins/log/](plugins/log/) as a separate project in the `thx::log` namespace, mirroring io: `log_interface` redefines its own `thx::log::LogLevel`/`LogRecord` and owns `thx::log::ILogService` plus the `thx::log::write`/`debug`/… facade (it does not depend on `<thx/log.h>`), `plugin_log_service` bridges the core sink to a registered `ILogService` (translating each core record), and `plugin_log_spdlog` is an spdlog backend. See CLAUDE.md "Errors & logging".

Still deferred: logging is **single-owner, replaced wholesale** — one `LogSink` and one `ILogService` at a time, no fan-out, no per-level/per-destination filtering, no scoped push/pop. **Direction:** a fan-out `ILogService` (the registered one multiplexes to N child backends) or a registry of providers; per-sink min-level filtering; maybe scoped push/pop for tests. **Trigger:** a consumer needs per-component filtering or multiple simultaneous destinations.
