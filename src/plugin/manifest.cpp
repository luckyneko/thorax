...
namespace thx::plugin
{
	...
	namespace
	{
		...
		bool parseRequirement(Reader& r, ManifestRequirement& out)
		{
			...
			if (r.peek() == '}')
				return false; // empty object, missing both fields
			...
			while (true)
			{
				...
				else if (key == "version")
				{
					...
					if (!parseString(r, vs))
						return false;
					if (!parseVersionTriple(vs, out.version))
						return false;
					haveVersion = true;
				}
				else
		...
		Result<PluginManifest, Error> parseDocument(Reader& r, const std::string& path)
		{
			...
			if (m.schema != 1)
				return Result<PluginManifest, Error>::err({ErrorCode::MalformedManifest,
														   "Manifest '" + path + "' uses unsupported schema " + std::to_string(m.schema) + " (this loader supports schema 1)"});
			if (!haveName)
				return Result<PluginManifest, Error>::err({ErrorCode::MalformedManifest,
