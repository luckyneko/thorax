...
namespace thx::plugin
{
	...
	PluginManager::PluginManager(ServiceManager& sm)
		: m_sm(sm)
	{
	}
		...
		for (std::size_t i = 0; i < reqs.size(); ++i)
		{
			...
			if (!svc)
				return Result<void, Error>::err({ErrorCode::NotLoaded,
												 std::string("plugin requires service '") + req.id.name() + "' which is not registered"});

			if (!Version::compatible(req.version, svc->version()))
			{
				std::string msg = std::string("plugin requires service '") + req.id.name() + "' at " + toString(req.version) + "; registered version is " + toString(svc->version());
		...
		// version
		{
			...
			if (liveVersion != opened.manifest.version)
				return mismatch("version",
								"manifest=" + toString(opened.manifest.version) + " live=" + toString(liveVersion));
		}
		...
		// requirements (compared as a set)
		{
			...
			if (liveSet != manifestSet)
				return mismatch("requirements",
								"manifest lists " + std::to_string(manifestSet.size()) + " requirement(s), IPlugin::required() reports " + std::to_string(liveSet.size()) + " (or contents differ)");
		}
		...
		// provides (compared as a set against actually-registered services)
		{
			...
			if (liveSet != manifestSet)
				return mismatch("provides",
								"manifest declares " + std::to_string(manifestSet.size()) + " service(s), plugin registered " + std::to_string(liveSet.size()) + " (or contents differ)");
		}
	...
	Result<void, Error> PluginManager::discover(const std::string& directory, Recursive recursive)
	{
		...
		auto handle = [&](const std::filesystem::directory_entry& entry)
		{
			...
			if (m_plugins.count(canonical) || m_opened.count(canonical) || m_discovered.count(canonical))
				return;
	...
	LoadSummary PluginManager::loadAll(Span<const PluginInfo> roots)
	{
		...
		for (const auto& p : order)
		{
			...
			{
				thx::logMessage(thx::LogLevel::Warn, "loadAll: failed to load '" + p + "': " + r.error().message);
				summary.failed.emplace_back(p, std::move(r.error()));
			}
		}
		return summary;
