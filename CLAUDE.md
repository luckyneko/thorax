...
examples/                a "media asset loader" suite: IAssetService + media-core/image/video decoder plugins, driven by five hosts (host, host_by_name, host_with_deps, host_logging, host_io) — one per pathway (host_logging selects a logger; host_io discovers + loads the io provider/file handler and round-trips a file through thx::io); integration tests run each
